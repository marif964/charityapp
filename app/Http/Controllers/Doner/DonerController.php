...
class DonerController extends Controller
{
    ...
    public function verificationNeeded(){
        ...
        $user = Auth::user();
        $verifyUser = Doner::where('id', $user->id)->first();
        
        if($verifyUser->email_verified_at != ''){
            return redirect()->route('doner.home');
        }
        
        return view('doner.verification-needed');
    }
    ...
    public function verified(Request $request){
        ...
        $verifyUser = Doner::where('remember_token', $remember_token)->first();
        ...
        if(!is_null($verifyUser) ){
            ...
            if($verifyUser->email_verified_at==null){
                ...
                $verifyUser->save();
                $message = "Thank you for activating your account. now you can sign in to your account";
                Auth::guard('doner')->logout();
                return redirect()->route('doner.login')->with(['successful_message' =>  $message]);
            } else {
                $message = "Your account is already verified.";
                
                return redirect()->route('doner.home')->with(['error_message' => $message]);
            }
        }
        
        return redirect()->route('verification-needed')->with(['error_message' => 'Sorry your email cannot be identified']);
    }
